...
- **Azure OpenAI** — LLM and embedding requests route through **Azure API Management (APIM)**, which acts as a gateway proxy. Both the OpenAI API key and an APIM subscription key header (`Ocp-Apim-Subscription-Key`) are sent on every request.
...
|----------|----------|-------------|
| `AZURE_OPENAI_ENDPOINT` | Yes | APIM gateway URL (e.g. `https://your-apim.azure-api.net`) |
...
| `APIM_SUBSCRIPTION_KEY` | Yes | Azure API Management subscription key |
| `AZURE_SEARCH_ENDPOINT` | Yes | Azure AI Search service URL |
| `AZURE_SEARCH_KEY` | Yes | Azure AI Search admin key |
...
- `api_key` — The Azure OpenAI API key (LangChain requires this for client validation)
- `azure_deployment` — The model deployment name
- `default_headers` — Adds `{"Ocp-Apim-Subscription-Key": "..."}` to every HTTP request

The `api_key` is needed because LangChain's Azure OpenAI client validates that a key is present. The `Ocp-Apim-Subscription-Key` header is what APIM actually uses for authentication and rate limiting at the gateway.
...
| APIM: both `api_key` and `Ocp-Apim-Subscription-Key` | LangChain requires a non-empty key for validation; APIM needs its own header |
